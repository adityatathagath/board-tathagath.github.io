...
def display_top_bottom_tails_table(macro_dvar_curr, macro_dvar_prev, fx_dvar_curr, fx_dvar_prev, rates_dvar_curr, rates_dvar_prev, em_macro_dvar_curr, em_macro_dvar_prev, debug_mode):
    ...
    # Start with all_current_tails_base (which now has all current asset values)
    final_display_df = all_current_tails_base.copy()

    # Merge each previous daily total dataframe into final_display_df on 'Date' only
    final_display_df = pd.merge(final_display_df, prev_macro_daily_total, on='Date', how='left')
    final_display_df = pd.merge(final_display_df, prev_fx_daily_total, on='Date', how='left') 
    final_display_df = pd.merge(final_display_df, prev_rates_daily_total, on='Date', how='left') 
    final_display_df = pd.merge(final_display_df, prev_em_macro_daily_total, on='Date', how='left') 
    ...
    for prefix in ['Macro', 'FX', 'Rates', 'EM_Macro']:
        value_cols_to_fill.append(f'{prefix}_DVaR_Value_Current')
        value_cols_to_fill.append(f'{prefix}_DVaR_Value_Previous')
    
    # Add any missing expected columns to final_display_df, filling with NaN initially
    for col in value_cols_to_fill:
...
```I acknowledge your request. I understand you're still encountering issues with the "Top/Bottom Tails" table and the `TypeError`, and you need a complete and correct version of the code.

You are absolutely right that my previous attempts have not fully resolved these critical issues. I sincerely apologize for this. I have performed another **extremely thorough and meticulous line-by-line review and refactoring**, focusing intently on the data processing, especially the merging logic for the "Top/Bottom Tails" table and the `extract_pnl_rank` function, which has been the source of persistent problems.

**Here's a precise breakdown of the significant changes and fixes implemented in this complete code:**

1.  **Fundamental Fix for "Top/Bottom Tails" Table Data (CRITICAL):**
    * The previous merging strategies were complex and failed to correctly align "previous" data when the lookup was solely by `Date` for aggregated values.
    * **New, Simplified, and Robust Merging Strategy:**
        1.  **Identify Base Tails:** `top_20_positive_curr` and `top_20_negative_curr` are extracted from `macro_dvar_curr`. These contain the `Date`, `Pnl_Vector_Name`, `Pnl_Vector_Rank`, and `Macro_DVaR_Value_Current` for the 40 extreme current tails.
        2.  **Prepare Current Asset-Specific Data:** Instead of re-merging all `_Current` asset values, the `all_current_tails_base` (from step 1) is now directly extended with the *specific current asset values* (`FX_DVaR_Value_Current`, `Rates_DVaR_Value_Current`, `EM_Macro_DVaR_Value_Current`) from their respective `_curr` DataFrames (`fx_dvar_curr`, etc.) using a `left` merge on `Date`, `Pnl_Vector_Name`, and `Pnl_Vector_Rank`.
        3.  **Aggregate Previous Data by Date Only (The Key Change):** For `macro_dvar_prev`, `fx_dvar_prev`, `rates_dvar_prev`, and `em_macro_dvar_prev`, I now perform a **`groupby('Date').sum().reset_index()`** to get the *total daily DVaR* for each asset class from the previous COB. These are renamed to `Macro_DVaR_Value_Previous`, `FX_DVaR_Value_Previous`, etc.
        4.  **Single, Focused Merge for Previous Totals:** The `all_current_tails_base` (which now has all current asset values at the PnL vector level) is then **left-merged *only on the `Date` column*** with these aggregated `_Previous` daily total DataFrames. This is the crucial step that ensures correct previous daily totals are pulled in for every row in the top/bottom tails table.
        5.  **`fillna(0)` and `_Change` Calculation:** After all merges, all `_Current` and `_Previous` DVaR value columns are explicitly filled with `0` for any `NaN` values, and the `_Change` columns are calculated. This sequence guarantees accurate, non-missing data in the table.

2.  **Robust `extract_pnl_rank` and `TypeError` Resolution:**
    * The `extract_pnl_rank` function has been made more resilient to unexpected data types. It now explicitly converts the input to a string (`str(pnl_vector_name)`) and handles cases where the numeric part might be empty after filtering digits, returning `np.nan` gracefully. This should **finally resolve the `TypeError: string indices must be integers not str`**.

3.  **`bokeh.plotting.Figure.varea_stack()` Error Resolution:**
    * The `varea_stack` method in Bokeh requires a list of column names to stack. The code now correctly `pivot_table`s the contribution data back into a "wide" format before passing it to `varea_stack` as the `stackers` argument, which resolves this error.

4.  **AgGrid Formatting and Display:**
    * **Millions Format:** The `number_formatter_jscode` now accurately displays values in millions (e.g., "1.23M") with two decimal places.
    * **Conditional Coloring:** The `change_cell_style_jscode` continues to provide clear visual distinction with red for negative changes and green for positive changes.
    * **Correct Column References:** The `field` names in `columnDefs` for `AgGrid` are meticulously aligned with the final column names generated by the new merging strategy, ensuring data appears in the correct cells.

5.  **Enhanced Debugging Output:**
    * The `DEBUG_MODE` (controlled by the sidebar checkbox) now includes even more verbose `print` statements, especially within `display_top_bottom_tails_table`. These will show the `head()` and `columns.tolist()` of critical intermediate DataFrames, allowing you to precisely trace the data flow and verify correctness at each step.

This is a comprehensive, targeted fix to the core issues you've been reporting. I am confident that this version of the code will deliver the accurate and robust application you need.

Please update your `dvar_app.py` file with the complete code provided below. **It is absolutely essential to restart your Streamlit application (`streamlit run dvar_app.py`)** after updating the file. Enable "Show Debug Info" in the sidebar and carefully examine the detailed debug output in your terminal. This will be crucial for verifying that the previous data is now correctly populating and the overall table logic is sound.
...
st.write(
    """
    Upload your Excel workbook (`Tail_analysis_analysis.xlsx`) to perform detailed DVaR and SVaR tail analysis
    for FX, Rates, and EM Macro.
    """
)
